...
class SHT30:
    ...
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
    ...
    def __init__(self, scl_pin=1, sda_pin=0, delta_temp=0, delta_hum=0, i2c_address=DEFAULT_I2C_ADDRESS):
        self.i2c = I2C(0,scl=Pin(scl_pin), sda=Pin(sda_pin),freq=400000)
        self.i2c_addr = i2c_address
        self.set_delta(delta_temp, delta_hum)
        time.sleep_ms(50)
        #print(self.measure_int())

    def init(self, scl_pin=1, sda_pin=0):
        """
        Init the I2C bus using the new pin values
        """
        self.i2c.init(0,scl=Pin(scl_pin), sda=Pin(sda_pin),freq=400000)
